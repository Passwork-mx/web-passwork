...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			coworking_create_observer( 'coworking-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				coworking_gutenberg_editor_init();
			} );
		} else {
		 	coworking_gutenberg_editor_init();
		}
	...
	function coworking_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'coworking_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'coworking_inited' ) ) {
	...
	// Init on page load
	function coworking_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = coworking_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + COWORKING_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + COWORKING_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in COWORKING_STORAGE['override_classes'] ) {
				$obj.addClass( COWORKING_STORAGE['override_classes'][i].replace( '%s', COWORKING_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( coworking_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && COWORKING_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'coworking_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			coworking_remove_observer( 'coworking-check-editor-styles-wrapper' );
			coworking_create_observer( 'coworking-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'coworking_inited' );
	}
} );
