...
jQuery( document ).ready(
	function() {
		...
		coworking_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="coworking_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.coworking_post_editor' );
						if ( link.length > 0 ) {
							coworking_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.coworking_post_editor', function(e) {
						coworking_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('coworking_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function coworking_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
