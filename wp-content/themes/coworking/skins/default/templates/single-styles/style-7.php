...
if ( apply_filters( 'coworking_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$coworking_with_featured_image = coworking_is_with_featured_image( $coworking_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $coworking_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $coworking_with_featured_image ) {
                echo ' with_featured_image' . ( false && coworking_get_theme_option( 'single_parallax' ) == 0 ? ' coworking-full-height' : '' );
            }
		?>">
			<?php
