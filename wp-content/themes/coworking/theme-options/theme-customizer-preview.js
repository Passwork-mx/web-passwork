...
( function( $ ) {
	...
	var $style = $( '#coworking-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length === 0 ) {
		var style_container = '<' + 'style type="text/css" id="coworking-customizer-inline-css" />',
			$theme_css      = $( '#coworking-custom-inline-css' );
		if ($theme_css.length === 0) {
			$theme_css = $( '#coworking-custom-css' );
		}
		if ($theme_css.length === 0) {
		...
		$style = $( '#coworking-customizer-inline-css' );
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id.indexOf( 'body_style' ) === 0) {
				...
				$( '.sc_layouts_logo .logo_text' ).html( coworking_prepare_macros( val ) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( coworking_prepare_macros( val ) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( coworking_prepare_macros( val ) );
				...
				if (id == 'front_page_bg_image') {
					...
					if (color_type == 'none') {
						...
						color = typeof coworking_color_schemes[scheme] != 'undefined' ? coworking_color_schemes[scheme]['colors']['bg_color'] : '';
					}
					$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : coworking_hex2rgba( color, mask ) ) );
					...
					.height( 'auto' )
					.toggleClass( 'coworking-full-height sc_layouts_flex sc_layouts_columns_middle', val );
					$( window ).trigger( 'resize' );
				}
			}
		} );
		...
		function add_color_scheme_helpers() {
			$( '[class*="scheme_"]:not(html)' ).each( function() {
				var $self = $( this ),
					...
					scheme = coworking_get_class_by_prefix( classes, 'scheme_' ).replace( 'scheme_', '' );
				$self.append( '<div class="coworking_customizer_scheme_helper">'
									+ '<span class="coworking_customizer_scheme_helper_name">'
										+ tag
										+ ( id
											? '#' + id
											: ( classes
		...
		function remove_color_scheme_helpers() {
			$( '.coworking_customizer_scheme_helper' ).remove();
		}
