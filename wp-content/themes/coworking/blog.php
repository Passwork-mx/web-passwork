...
if ( function_exists( 'coworking_elementor_is_preview' ) && coworking_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			coworking_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	coworking_new_main_query(
		array(
			'post_type'      => coworking_get_theme_option( 'post_type' ),
			'category'       => coworking_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'coworking_filter_get_template_part', coworking_blog_archive_get_template() ) );
}
