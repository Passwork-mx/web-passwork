...
if ( ! function_exists( 'coworking_storage_get' ) ) {
	/**
	 ...
	 */
	function coworking_storage_get( $var_name, $default = '' ) {
		global $COWORKING_STORAGE;
		return isset( $COWORKING_STORAGE[ $var_name ] ) ? $COWORKING_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'coworking_storage_set' ) ) {
	/**
	 ...
	 */
	function coworking_storage_set( $var_name, $value ) {
		global $COWORKING_STORAGE;
		$COWORKING_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'coworking_storage_empty' ) ) {
	/**
	 ...
	 */
	function coworking_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $COWORKING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $COWORKING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $COWORKING_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $COWORKING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'coworking_storage_isset' ) ) {
	/**
	 ...
	 */
	function coworking_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $COWORKING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $COWORKING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $COWORKING_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $COWORKING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'coworking_storage_unset' ) ) {
	/**
	 ...
	 */
	function coworking_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $COWORKING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $COWORKING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $COWORKING_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $COWORKING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'coworking_storage_inc' ) ) {
	/**
	 ...
	 */
	function coworking_storage_inc( $var_name, $value = 1 ) {
		global $COWORKING_STORAGE;
		if ( empty( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = 0;
		}
		$COWORKING_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'coworking_storage_concat' ) ) {
	/**
	 ...
	 */
	function coworking_storage_concat( $var_name, $value ) {
		global $COWORKING_STORAGE;
		if ( empty( $COWORKING_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'coworking_storage_get_array' ) ) {
	/**
	 ...
	 */
	function coworking_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $COWORKING_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $COWORKING_STORAGE[ $var_name ][ $key ] ) ? $COWORKING_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $COWORKING_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $COWORKING_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'coworking_storage_set_array' ) ) {
	/**
	 ...
	 */
	function coworking_storage_set_array( $var_name, $key, $value ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$COWORKING_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'coworking_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function coworking_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $COWORKING_STORAGE[ $var_name ][ $key ] ) ) {
			$COWORKING_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$COWORKING_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$COWORKING_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'coworking_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function coworking_storage_merge_array( $var_name, $key, $value ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$COWORKING_STORAGE[ $var_name ] = array_merge( $COWORKING_STORAGE[ $var_name ], $value );
		} else {
			$COWORKING_STORAGE[ $var_name ][ $key ] = array_merge( $COWORKING_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'coworking_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function coworking_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			coworking_array_insert_after( $COWORKING_STORAGE[ $var_name ], $after, $key );
		} else {
			coworking_array_insert_after( $COWORKING_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'coworking_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function coworking_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			coworking_array_insert_before( $COWORKING_STORAGE[ $var_name ], $before, $key );
		} else {
			coworking_array_insert_before( $COWORKING_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'coworking_storage_push_array' ) ) {
	/**
	 ...
	 */
	function coworking_storage_push_array( $var_name, $key, $value ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $COWORKING_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $COWORKING_STORAGE[ $var_name ][ $key ] ) ) {
				$COWORKING_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $COWORKING_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'coworking_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function coworking_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $COWORKING_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $COWORKING_STORAGE[ $var_name ] ) && is_array( $COWORKING_STORAGE[ $var_name ] ) && count( $COWORKING_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $COWORKING_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $COWORKING_STORAGE[ $var_name ][ $key ] ) && is_array( $COWORKING_STORAGE[ $var_name ][ $key ] ) && count( $COWORKING_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $COWORKING_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'coworking_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function coworking_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( empty( $COWORKING_STORAGE[ $var_name ][ $key ] ) ) {
			$COWORKING_STORAGE[ $var_name ][ $key ] = 0;
		}
		$COWORKING_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'coworking_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function coworking_storage_concat_array( $var_name, $key, $value ) {
		global $COWORKING_STORAGE;
		if ( ! isset( $COWORKING_STORAGE[ $var_name ] ) ) {
			$COWORKING_STORAGE[ $var_name ] = array();
		}
		if ( empty( $COWORKING_STORAGE[ $var_name ][ $key ] ) ) {
			$COWORKING_STORAGE[ $var_name ][ $key ] = '';
		}
		$COWORKING_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'coworking_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function coworking_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $COWORKING_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $COWORKING_STORAGE[ $var_name ] ) ? $COWORKING_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $COWORKING_STORAGE[ $var_name ] ) ? $COWORKING_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'coworking_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function coworking_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $COWORKING_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $COWORKING_STORAGE[ $var_name ]->$prop ) ? $COWORKING_STORAGE[ $var_name ]->$prop : $default;
	}
}
