...
jQuery( document ).on( 'action.ready_coworking', function() {
		...
		if ( $product.length ) {
			...
			if ( $gallery.length ) {
				$document.on( 'action.resize_coworking', function() {
					coworking_woocommerce_stretch_fullwidth();
				} );
				coworking_woocommerce_stretch_fullwidth();
	...
	function coworking_woocommerce_stretch_fullwidth() {
	...
	if ( $body.hasClass('single-product') ) {
		...
		if ( product_wrap.length ) {
			...
			coworking_add_filter('coworking_filter_masonry_wrap', function( selector ) {
				selector += ( selector ? ',' : '' ) + '.woocommerce-product-gallery__wrapper';
				return selector;
			});
			// Add gallery image to the masonry item selector
			coworking_add_filter('coworking_filter_masonry_item', function( selector ) {
	...
	if ( COWORKING_STORAGE['add_to_cart_sticky'] ) {

		// Show sticky bar on form is out of the window viewport
		var $form = jQuery( '.summary form.cart' );
		if ( $form.length ) {
			coworking_intersection_observer_add( $form, function( item, enter, data ) {
				if ( ! enter ) {
		...
		if ( $footer.length ) {
			coworking_intersection_observer_add( $footer, function( item, enter, data ) {
				if ( ! enter ) {
					if ( ! $body.hasClass( 'single_product_bottom_bar_sticky_on' ) ) {
						$body.addClass( 'single_product_bottom_bar_sticky_on' );
					}
		...
		jQuery( '.single_product_bottom_bar_button_select_options' ).on( 'click', function(e) {
			coworking_document_animate_to( '.variations_form' );
			e.preventDefault();
			return false;
		} );
	}
	...
	jQuery( document ).on( 'action.init_hidden_elements', function( e, $cont ) {
		$cont.find( '.coworking_woocommerce_filters_button:not(.inited),.sidebar > .sidebar_close:not(.inited),.sidebar_overlay:not(.inited)' )
			.addClass( 'inited' )
			.on( 'click', function(e) {
