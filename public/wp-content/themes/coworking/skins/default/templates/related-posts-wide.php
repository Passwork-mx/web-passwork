...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $coworking_post_format ) ); ?> data-post-id="<?php the_ID(); ?>">
	...
	coworking_show_post_featured(
		array(
			'thumb_size'    => apply_filters( 'coworking_filter_related_thumb_size', coworking_get_thumb_size( (int) coworking_get_theme_option( 'related_posts' ) == 1 ? 'big' : 'med' ) ),
		)
	);
	?>
	<div class="post_header entry-header">
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<div class="post_meta">
				<a href="<?php echo esc_url( $coworking_link ); ?>" class="post_meta_item post_date"><?php echo wp_kses_data( coworking_get_date() ); ?></a>
			</div>
			<?php
		}
		?>
