...
var id = settings._element_id ? settings._element_id + '_sc' : 'sc_icons_'+(''+Math.random()).replace('.', ''),
    svg_bg_dir = '<?php echo trx_addons_get_svg_from_file(coworking_get_file_dir('images/decor.svg')); ?>',
    number = 0;
	...
	_.each(settings.icons, function(item) {
        ...
        #><div class="sc_icons_item<# if (item.link.url != '' ) print(' sc_icons_item_linked')
            ...
            if (item.char != '') {
                ...
                #><div class="sc_icons_image"><img src="{{ item.image.url }}" alt="<?php esc_attr_e('Icon', 'coworking'); ?>"></div><#
                    if ( settings.type == 'creative' && svg_bg_dir != '' )  {
                        #><div class="sc_icons_decoration_bg"><# print(svg_bg_dir); #></div><#
                    }
            } else {
                ...
                if (icon != '') {
                    ...
                    #>><#
                        if (svg != '') {
                            ...
                            #><img class="sc_icon_as_image" src="{{ img }}" alt="<?php esc_attr_e('Icon', 'coworking'); ?>"><#
                        } else {
