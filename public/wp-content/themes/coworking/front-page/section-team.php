<div class="front_page_section front_page_section_team<?php
	$coworking_scheme = coworking_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $coworking_scheme ) && ! coworking_is_inherit( $coworking_scheme ) ) {
		echo ' scheme_' . esc_attr( $coworking_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( coworking_get_theme_option( 'front_page_team_paddings' ) );
	if ( coworking_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$coworking_css      = '';
		$coworking_bg_image = coworking_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$coworking_anchor_icon = coworking_get_theme_option( 'front_page_team_anchor_icon' );
	$coworking_anchor_text = coworking_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( coworking_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$coworking_css      = '';
			$coworking_bg_mask  = coworking_get_theme_option( 'front_page_team_bg_mask' );
			$coworking_bg_color_type = coworking_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $coworking_bg_color_type ) {
				$coworking_bg_color = coworking_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$coworking_caption = coworking_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $coworking_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $coworking_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $coworking_caption, 'coworking_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$coworking_description = coworking_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $coworking_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $coworking_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $coworking_description ), 'coworking_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! coworking_exists_trx_addons() ) {
						...
						coworking_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
